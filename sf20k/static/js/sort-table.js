...
function sortTable(Table, col, dir) {
    ...
    if (null != sortClass) {
        ...
        var firstRowCell = Table.tBodies[0].rows[0]?.cells[col];
        if (firstRowCell && !isNaN(parseFloat(sortTable.stripTags(firstRowCell.innerHTML)))) {
            sortTable.sortFunc = 'number';
        } else {
...
sortTable.compareRow = function(RowA, RowB) {
    ...
    if (typeof sortTable[sortTable.sortFunc] !== 'function') {
        sortTable.sortFunc = 'string';
    }

    // Check if the column is for model names
    if (sortTable.sortFunc === 'model') {
        // Extract text from the <a> tag within the column for model names
        const getModelName = (Cell) => {
            const link = Cell.querySelector('a');
            return link ? link.textContent.trim() : ''; // Fallback to empty string if no link
        };

        valA = getModelName(RowA.cells[sortTable.sortCol]);
        valB = getModelName(RowB.cells[sortTable.sortCol]);
    } else {
        // Use the specified sort function for numeric or string values
        ...
        // Handle '-' or non-numeric values (put them last)
        if (valA === '-' || isNaN(valA)) valA = Number.MIN_SAFE_INTEGER;
        if (valB === '-' || isNaN(valB)) valB = Number.MIN_SAFE_INTEGER;
    }
